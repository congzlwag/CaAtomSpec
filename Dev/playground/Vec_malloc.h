...
public:
    Vec(): p(NULL),n(0){}
    Vec(const size_t & m): p((T*)std::malloc(m*sizeof(T))), n(m){}
    Vec(const size_t & m, const T & x): p((T*)std::malloc(m*sizeof(T))), n(m){for(size_t i=0; i<m; i++) p[i]=x;}
    Vec(const Vec & v1): p((T*)std::malloc((v1.siz())*sizeof(T))), n(v1.siz()){for(size_t i=0; i<n; i++) p[i]=v1[i];}
    size_t siz() const {return n;}
    void resiz(const size_t & nn){
        if(n!=nn){
            T* pnew = (T*)std::malloc(nn*sizeof(T));
